...
- **`/likes` — реализовано**: `GET /likes` (пагинация, JOIN tracks, ORDER BY created_at DESC), `PUT /likes/{track_id}` (201 новый / 200 идемпотентный повтор), `DELETE /likes/{track_id}` (204, идемпотентен).
